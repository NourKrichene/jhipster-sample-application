...
describe('Project Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call Project query and add missing value', () => {
      ...
      const creator: IProject = { id: 9290 };
      project.creator = creator;

      const projectCollection: IProject[] = [{ id: 43807 }];
      jest.spyOn(projectService, 'query').mockReturnValue(of(new HttpResponse({ body: projectCollection })));
      const additionalProjects = [creator];
    ...
    it('Should update editForm', () => {
      const project: IProject = { id: 456 };
      const creator: IProject = { id: 25972 };
      project.creator = creator;

      activatedRoute.data = of({ project });
      comp.ngOnInit();

      expect(comp.projectsSharedCollection).toContain(creator);
